...
def gen(camera):
		...
		if process_this_frame:
			...
			for face_encoding in face_encodings:
				...
				if matches[best_match_index]:
					...
					r = requests.post(url='https://testfacedetect.herokuapp.com/receive_data', json=json_to_export)
	...
	while True:
		...
		path_file = os.path.join(f"static/assets/img/image_for_add_emp/capture.jpg")
		...
		try:
			...
			if result:
			   ...
			   image_path = f"static/assets/img/departures/{json_data['date']}/{json_data['name']}/departure.jpg"
				...
				# Save image
			   os.makedirs(f"{FILE_PATH}/static/assets/img/departures/{json_data['date']}/{json_data['name']}", exist_ok=True)
			   ...
			   cursor.execute(update_user_querry)
				...
				os.makedirs(f"{FILE_PATH}/static/assets/img/arrivals/{json_data['date']}/{json_data['name']}", exist_ok=True)
				cv2.imwrite(image_path, np.array(json_data['picture_array']))
				json_data['picture_path'] = image_path
...
def get_employee(name):
	...
	keys=[ "empName",  "date", "inTime", "outTime",  "inTimeImg", "outTimeImg", "id"]
...
def get_employee_list():
	...
	walk_count = 0
	for file_name in os.listdir(f"{FILE_PATH}/static/assets/img/users/"):
...
def delete_employee(name):
	try:
		...
		print('name: ', name)
		file_path = os.path.join(f'static/assets/img/users/{name}.jpg')
		os.remove(file_path)
		answer = 'success'
	except:
		answer = 'error'
